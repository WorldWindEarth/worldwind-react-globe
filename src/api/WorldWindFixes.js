...
const NETWORK_RETRIEVAL_QUEUE_SIZE = 8;
const MEMORY_CACHE_VOLATILE_BIAS = 60e3;   // in milliseconds

export default class WorldWindFixes {
    
    /**
     * Records if applyLibraryFixes has been run.
     */
...
//                console.log('putEntry( ' + (this.usedCapacity / this._capacity * 100).toFixed(0) + '% )');
            ...
            WorldWind.TiledImageLayer.prototype.retrieveTileImage = function (dc, tile, suppressRedraw) {
                if (this.currentRetrievals.indexOf(tile.imagePath) < 0) {
                    if (this.currentRetrievals.length > NETWORK_RETRIEVAL_QUEUE_SIZE) {
                        return;
                    }
     ...
     */
    static applyWorldWindowFixes(wwd) {
        // Increase size to prevent thrashing tile cache at oblique view from the surface
        wwd.drawContext.surfaceShapeTileBuilder.tileCache.capacity = TILE_CACHE_CAPACITY;
        wwd.drawContext.surfaceShapeTileBuilder.tileCache.lowWater = TILE_CACHE_LOW_WATER;
    }

}

export {WorldWindFixes, TILE_CACHE_CAPACITY, TILE_CACHE_LOW_WATER, NETWORK_RETRIEVAL_QUEUE_SIZE, MEMORY_CACHE_VOLATILE_BIAS};